...
func (p *ProgramConfig) Setup() {
	...
	for _, p := range p.proxy {
		netutil.AddProxy(netutil.GlobalProxies, p.String())
	}
	// 2. add apis
	for _, ag := range p.ags {
		api := ag.Generate()
		netutil.ApiMap.Add2Apis(ag.apiName, api)
	}
...
func (a *ApiGenerator) Generate() netutil.Api {
	copiedA := *a
	f := func(t netutil.Type) (string, error) {
		var URL string
		switch t {
		case netutil.A:
			URL = copiedA.a
		case netutil.AAAA:
			URL = copiedA.aaaa
		default:
			return "", netutil.NewUnknownType(t)
		}
...
func (t txtHandler) HandleResponse(source string, no string) (target any, err error) {
	if matches := netutil.IpPattern.FindAllString(source, -1); matches != nil {
